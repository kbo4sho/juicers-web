...
function drawHand(
  ...
  const pulse = closed ? 1.08 : 1 + Math.sin(time * 0.006) * 0.035;
  context.save();
  context.translate(x, y);
  context.scale(pulse, pulse);
  context.shadowBlur = closed ? 28 : 16;
  context.shadowColor = closed ? "#fff46d" : "#68f4d2";
  context.strokeStyle = closed ? "#fff585" : "#7ffadd";
  context.fillStyle = closed ? "rgba(255,202,51,.28)" : "rgba(32,20,55,.4)";
  context.lineWidth = closed ? 7 : 4;
  context.beginPath();
  context.arc(0, 0, closed ? 34 : 42, 0, Math.PI * 2);
  context.fill();
  context.stroke();
  context.shadowBlur = 0;
  if (closed) {
    context.fillStyle = "rgba(255,255,255,.94)";
    roundedRect(context, -17, -13, 34, 27, 9);
    context.fill();
    context.strokeStyle = "#e0a72d";
    context.lineWidth = 3;
    ...
    for (let index = -2; index <= 2; index += 1) {
      context.beginPath();
      context.moveTo(index * 7, 12);
      context.lineTo(index * 9, -14 - Math.abs(index) * 2);
      context.stroke();
    }
  }
  context.fillStyle = "rgba(19,9,34,.88)";
  context.beginPath();
  context.arc(31, 30, 11, 0, Math.PI * 2);
  context.fill();
  context.fillStyle = "#fff";
  context.font = "800 10px system-ui";
  context.textAlign = "center";
  context.textBaseline = "middle";
  context.fillText(id === "left" ? "L" : "R", 31, 30);
  context.restore();
}
